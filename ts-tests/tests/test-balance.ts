...
describeWithAcala("Acala RPC (Balance)", (context) => {
	...
	step("balance to be updated after transfer", async function () {
		...
		expect((await context.provider.getBalance(alice.getAddress())).toString()).to.equal("8999999986279096000000000");
		expect((await context.provider.getBalance(alice_stash.getAddress())).toString()).to.equal("10100000986279101031000000");

		await transfer(context, await alice.getSubstrateAddress(), await alice_stash.getSubstrateAddress(), 1000);
		expect((await context.provider.getBalance(alice.getAddress())).toString()).to.equal("8999999969511494609000000");
