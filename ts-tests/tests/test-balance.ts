...
describeWithAcala("Acala RPC (Balance)", (context) => {
    ...
    step("balance to be updated after transfer", async function () {
        ...
        expect((await context.provider.getBalance(alice.getAddress())).toString()).to.equal("8999999992132571345000000");
        expect((await context.provider.getBalance(alice_stash.getAddress())).toString()).to.equal("10100000995937432778000000");
        expect((await context.provider.getBalance(alice.getAddress(), "latest")).toString()).to.equal("8999999992132571345000000");
