#!/usr/bin/env bash

set -e

if [ $1 = "karura" ]; then
	IMAGE=acala/karura-node:1.0.1
	CHAIN_NAME=karura
	GENESIS=./resources/karura-dist.json
elif [ $1 = "acala" ]; then
	IMAGE=acala/acala-node:1.0.1
	CHAIN_NAME=acala
	GENESIS=
else
	echo "not support $1"
	exit 1
fi

docker rm -f bootnotes > /dev/null 2>&1 || true

list=`cat $GENESIS |jq -r .bootNodes[]`

for node in $list;
do
	#echo $node
	docker run \
		   -p 9933:9933 \
		   -d \
		   --rm \
		   --name bootnotes \
		   $IMAGE \
		   --chain=$CHAIN_NAME \
		   --reserved-only \
		   --reserved-nodes=$node \
		   --rpc-port=9933 \
		   --rpc-external=true \
		   --rpc-cors=all \
		   --rpc-methods=unsafe \
		   -- \
		   --chain=dev \
		> /dev/null

	sleep 8s
	peer=$(curl -sS \
         -H 'Content-Type: application/json' \
         --data '{"id":1,"jsonrpc":"2.0","method":"system_peers"}' \
         localhost:9933 |\
		jq -r '.result')

	peer_num=$(echo $peer | jq 'length')
	peer_id=$(echo $peer | jq -r '.[0].peerId')

	if [ "$peer_num" != "1" ]; then
		echo "Connect to $node failed. Node connect to peer number is: $peer_num"
	elif [ $peer_id != ${node##*/} ]; then
		echo "Connect to $node failed. The actual peer id: $peer_id"
	fi

	docker rm -f bootnotes > /dev/null
done
