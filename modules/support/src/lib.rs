...
use primitives::{task::TaskResult, Balance, CurrencyId, IncomeSource, Multiplier, ReserveIdentifier};
use sp_runtime::{
...
impl<AccountId> LiquidateCollateral<AccountId> for Tuple {
	fn liquidate(
		...
		for_tuples!( #(
			match Tuple::liquidate(who, currency_id, amount, target_base_amount, target_penalty_amount) {
				Ok(_) => return Ok(()),
