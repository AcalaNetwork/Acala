...
use mock::{
	dollar, millicent, Currencies, ExtBuilder, HomaLite, MockRelayBlockNumberProvider, Runtime, RuntimeEvent,
	RuntimeOrigin, System, ACALA, ALICE, BOB, CHARLIE, DAVE, INITIAL_BALANCE, INVALID_CALLER, KSM, LKSM,
};
...
fn mint_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			HomaLite::mint(RuntimeOrigin::signed(DAVE), amount),
		...
		assert_ok!(HomaLite::mint(RuntimeOrigin::signed(ALICE), amount));
		...
		assert_ok!(HomaLite::mint(RuntimeOrigin::signed(BOB), amount));
...
fn mint_fails_when_cap_is_exceeded() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(HomaLite::set_minting_cap(RuntimeOrigin::root(), dollar(1_000)));

		assert_noop!(
			HomaLite::mint(RuntimeOrigin::signed(ALICE), dollar(1_001)),
		...
		assert_noop!(
			HomaLite::mint(RuntimeOrigin::signed(ALICE), dollar(1)),
			Error::<Runtime>::ExceededStakingCurrencyMintCap
		);
	});
}
...
fn failed_xcm_transfer_is_handled() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			HomaLite::mint(RuntimeOrigin::signed(INVALID_CALLER), dollar(1)),
...
fn cannot_set_total_staking_currency_to_zero() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			HomaLite::set_total_staking_currency(RuntimeOrigin::root(), 0),
			Error::<Runtime>::InvalidTotalStakingCurrency
		);
		assert_ok!(HomaLite::set_total_staking_currency(RuntimeOrigin::root(), 1));
		assert_eq!(TotalStakingCurrency::<Runtime>::get(), 1);
		System::assert_last_event(RuntimeEvent::HomaLite(crate::Event::TotalStakingCurrencySet {
...
fn can_adjust_total_staking_currency() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(HomaLite::adjust_total_staking_currency(RuntimeOrigin::root(), 5000i128));
		...
		assert_ok!(HomaLite::adjust_total_staking_currency(RuntimeOrigin::root(), 1i128));
		assert_eq!(HomaLite::total_staking_currency(), Balance::max_value());

		// Do not allow TotalStakingCurrency to become 0
		assert_ok!(HomaLite::set_total_staking_currency(RuntimeOrigin::root(), 5000));
		assert_noop!(
			HomaLite::adjust_total_staking_currency(RuntimeOrigin::root(), -5000i128),
			Error::<Runtime>::InvalidTotalStakingCurrency
		);
...
fn can_adjust_available_staking_balance_with_no_matches() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			HomaLite::adjust_available_staking_balance(RuntimeOrigin::signed(ALICE), 5000i128, 10),
			BadOrigin
		);
		...
		System::assert_last_event(RuntimeEvent::HomaLite(crate::Event::AvailableStakingBalanceSet {
...
fn requires_root_to_set_total_staking_currency() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			HomaLite::set_total_staking_currency(RuntimeOrigin::signed(ALICE), 0),
			BadOrigin
		);
	});
}
...
fn can_set_mint_cap() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			HomaLite::set_minting_cap(RuntimeOrigin::signed(ALICE), dollar(1_000)),
			BadOrigin
		);
		...
		System::assert_last_event(RuntimeEvent::HomaLite(crate::Event::StakingCurrencyMintCapUpdated {
...
fn can_set_xcm_dest_weight() {
	ExtBuilder::default().build().execute_with(|| {
		// Requires Root privilege.
		assert_noop!(
			HomaLite::set_xcm_dest_weight(RuntimeOrigin::signed(ALICE), Weight::from_ref_time(1_000_000)),
			BadOrigin
		);
...
fn can_schedule_unbond() {
	ExtBuilder::default().build().execute_with(|| {
		// Requires Root privilege.
		assert_noop!(
			HomaLite::schedule_unbond(RuntimeOrigin::signed(ALICE), 1_000_000, 100),
			BadOrigin
		);

		// Schedule an unbond.
		assert_ok!(HomaLite::schedule_unbond(RuntimeOrigin::root(), 1_000_000, 100));

		// Storage should be updated now.
		assert_eq!(ScheduledUnbond::<Runtime>::get(), vec![(1_000_000, 100)]);

		System::assert_last_event(RuntimeEvent::HomaLite(crate::Event::ScheduledUnbondAdded {
			staking_amount: 1_000_000,
			relaychain_blocknumber: 100,
		}));

		// Schedule another unbond.
		assert_ok!(HomaLite::schedule_unbond(RuntimeOrigin::root(), 200, 80));
...
fn can_replace_schedule_unbond() {
	ExtBuilder::default().build().execute_with(|| {
		// Requires Root privilege.
		assert_noop!(
			HomaLite::replace_schedule_unbond(RuntimeOrigin::signed(ALICE), vec![(1_000_000, 100)]),
			BadOrigin
		);
		...
		System::assert_last_event(RuntimeEvent::HomaLite(crate::Event::ScheduledUnbondReplaced));
	});
}
...
fn on_idle_can_process_xcm_to_increase_available_staking_balance() {
	ExtBuilder::default().build().execute_with(|| {
		...
		HomaLite::on_idle(
			MockRelayBlockNumberProvider::get(),
			Weight::from_ref_time(5_000_000_000),
		);
		assert_eq!(ScheduledUnbond::<Runtime>::get(), vec![(100, 1), (200, 2), (30, 2)]);
		assert_eq!(AvailableStakingBalance::<Runtime>::get(), 0);
...
fn request_redeem_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			HomaLite::request_redeem(RuntimeOrigin::signed(DAVE), dollar(1), Permill::zero()),
		...
		assert_noop!(
			HomaLite::request_redeem(RuntimeOrigin::signed(DAVE), dollar(1_000_001), Permill::zero()),
			orml_tokens::Error::<Runtime>::BalanceTooLow
		);
...
fn update_redeem_request_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let events = System::events()
			.into_iter()
			.filter_map(|e| match e.event {
				RuntimeEvent::HomaLite(x) => Some(RuntimeEvent::HomaLite(x)),
				RuntimeEvent::Tokens(orml_tokens::Event::Unreserved {
					currency_id: currency,
					who,
					amount,
				}) => Some(RuntimeEvent::Tokens(orml_tokens::Event::Unreserved {
					currency_id: currency,
					who,
					amount,
				})),
				RuntimeEvent::Tokens(orml_tokens::Event::Reserved {
					currency_id: currency,
					who,
					amount,
				}) => Some(RuntimeEvent::Tokens(orml_tokens::Event::Reserved {
					currency_id: currency,
					who,
					amount,
				})),
		...
		assert_eq!(
			...
			vec![
				...
				RuntimeEvent::HomaLite(crate::Event::RedeemRequested {
					who: DAVE,
...
fn skip_redeem_requests_if_not_enough_reserved_liquid_currency() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(HomaLite::mint(RuntimeOrigin::signed(ALICE), dollar(40)));
...
fn mint_can_handle_dust_redeem_requests() {
	ExtBuilder::empty().build().execute_with(|| {
		...
		assert_ok!(HomaLite::mint(RuntimeOrigin::signed(BOB), mint_amount));
...
fn mint_can_match_requested_redeem() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(HomaLite::mint(RuntimeOrigin::signed(CHARLIE), dollar(100)));
...
fn request_redeem_extra_fee_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(HomaLite::mint(RuntimeOrigin::signed(CHARLIE), dollar(30)));
...
fn total_staking_currency_update_periodically() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			HomaLite::set_staking_interest_rate_per_update(RuntimeOrigin::signed(ALICE), Permill::from_percent(1)),
			BadOrigin
		);
		...
		System::assert_last_event(RuntimeEvent::HomaLite(crate::Event::StakingInterestRatePerUpdateSet {
...
fn process_scheduled_unbond_with_multiple_requests() {
	ExtBuilder::empty().build().execute_with(|| {
		...
		assert_ok!(HomaLite::set_total_staking_currency(RuntimeOrigin::root(), dollar(40)));
...
fn not_overcharge_redeem_fee() {
	ExtBuilder::empty().build().execute_with(|| {
		...
		assert_ok!(HomaLite::set_total_staking_currency(RuntimeOrigin::root(), dollar(10)));
...
fn on_idle_matches_redeem_based_on_weights() {
	ExtBuilder::default().build().execute_with(|| {
		...
		// Schedule an unbond.
		assert_ok!(HomaLite::schedule_unbond(RuntimeOrigin::root(), dollar(1_000_000), 0));
		MockRelayBlockNumberProvider::set(0);
		...
		assert_eq!(
			HomaLite::on_idle(MockRelayBlockNumberProvider::get(), Weight::zero()),
			Weight::zero()
		);
		...
		assert_eq!(
			HomaLite::on_idle(
				MockRelayBlockNumberProvider::get(),
				xcm_weight + Weight::from_ref_time(1)
			),
			xcm_weight
		);
		assert_eq!(ScheduledUnbond::<Runtime>::get(), vec![]);
		...
		assert_eq!(
			HomaLite::on_idle(
				MockRelayBlockNumberProvider::get(),
				xcm_weight + redeem + Weight::from_ref_time(1)
			),
			xcm_weight + redeem
		);
		assert_eq!(ScheduledUnbond::<Runtime>::get(), vec![]);
		...
		assert_ok!(HomaLite::schedule_unbond(RuntimeOrigin::root(), dollar(1_000_000), 10));
		assert_eq!(ScheduledUnbond::<Runtime>::get(), vec![(dollar(1_000_000), 10)]);
		assert_eq!(
			HomaLite::on_idle(MockRelayBlockNumberProvider::get(), redeem + Weight::from_ref_time(1)),
			redeem
		);
		assert_eq!(ScheduledUnbond::<Runtime>::get(), vec![(dollar(1_000_000), 10)]);
		assert_eq!(RedeemRequests::<Runtime>::get(DAVE), None);
...
fn adjust_available_staking_balance_matches_redeem_based_on_input() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(HomaLite::adjust_available_staking_balance(RuntimeOrigin::root(), 1, 10));
		assert_eq!(RedeemRequests::<Runtime>::get(DAVE), None);
		assert_eq!(RedeemRequests::<Runtime>::get(ALICE), None);
		assert_eq!(RedeemRequests::<Runtime>::get(BOB), None);
	});
...
fn mint_can_handle_rounding_error_dust() {
	ExtBuilder::empty().build().execute_with(|| {
		...
		assert_ok!(HomaLite::mint(RuntimeOrigin::signed(DAVE), 999_999_999_999,));
