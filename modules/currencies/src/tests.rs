...
fn fungible_inspect_trait_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(<NativeCurrency as fungible::Inspect<_>>::minimum_balance(), 1);
			assert_eq!(<AdaptedBasicCurrency as fungible::Inspect<_>>::minimum_balance(), 1);
			...
			assert_eq!(<NativeCurrency as fungible::Inspect<_>>::balance(&alice()), 51500);
			assert_eq!(<AdaptedBasicCurrency as fungible::Inspect<_>>::balance(&alice()), 51500);
			...
			assert!(
				<Currencies as fungibles::Inspect<_>>::can_deposit(NATIVE_CURRENCY_ID, &alice(), 0)
					== <AdaptedBasicCurrency as fungible::Inspect<_>>::can_deposit(&alice(), 0)
			);
			assert!(
			...
			assert!(
				<Currencies as fungibles::Inspect<_>>::can_deposit(X_TOKEN_ID, &alice(), 0)
					== <Tokens as fungibles::Inspect<_>>::can_deposit(X_TOKEN_ID, &alice(), 0)
			);
			assert!(
...
fn fungible_mutate_trait_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			System::assert_last_event(Event::Currencies(crate::Event::Deposited(X_TOKEN_ID, alice(), 1000)));
			assert_eq!(
				<Currencies as fungibles::Inspect<_>>::total_issuance(X_TOKEN_ID),
				201000
			);
			...
			System::assert_last_event(Event::Currencies(crate::Event::Withdrawn(X_TOKEN_ID, alice(), 1000)));
			assert_eq!(
				<Currencies as fungibles::Inspect<_>>::total_issuance(X_TOKEN_ID),
				200000
			);
...
fn fungible_inspect_hold_and_hold_trait_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				<Currencies as fungibles::InspectHold<_>>::can_hold(NATIVE_CURRENCY_ID, &alice(), 20000),
				true
			);
			...
			assert_eq!(
				<Currencies as fungibles::InspectHold<_>>::can_hold(X_TOKEN_ID, &alice(), 20000),
				true
			);
			assert_ok!(<Currencies as fungibles::MutateHold<_>>::hold(
				X_TOKEN_ID,
			...
			// TODO: should be Ok(2000), should fix orml_tokens first
			assert_eq!(
				<Currencies as fungibles::MutateHold<_>>::transfer_held(
					X_TOKEN_ID,
					&alice(),
