...
//!
//! Reward accumulation:
//! 1. LoansIncentive/DexIncentive/DexSaving: the fixed
...
pub enum PoolId<AccountId> {
	/// Rewards for users who open CDP
	LoansIncentive {
			...
			if !T::EmergencyShutdown::is_shutdown() && now % T::AccumulatePeriod::get() == Zero::zero() {
				...
				for (pool_id, pool_info) in orml_rewards::Pools::<T>::iter() {
					if !pool_info.total_shares.is_zero() {
						match pool_id {
							PoolId::LoansIncentive { reward_currency_id, .. } => {
								count += 1;
		...
		pub fn claim_rewards(origin: OriginFor<T>, pool_id: PoolId<T::RelaychainAccountId>) -> DispatchResult {
			...
			if !pending_reward.is_zero() {
				let currency_id = match pool_id {
					PoolId::LoansIncentive { reward_currency_id, .. } => reward_currency_id,
					PoolId::DexIncentive(_) | PoolId::Unused => T::NativeCurrencyId::get(),
...
impl<T: Config> Pallet<T> {
	...
	fn accumulate_incentives(pool_id: PoolId<T::RelaychainAccountId>, reward_currency_id: CurrencyId) {
		let source = T::RewardsSource::get();
			...
			match res {
				...
				Err(e) => {
					log::warn!(
						...
						incentive_reward_amount, reward_currency_id, source, Self::account_id(), e
					);
				}
			}
		}
	...
	fn accumulate_dex_saving(pool_id: PoolId<T::RelaychainAccountId>, lp_currency_id: CurrencyId) {
		...
		if !dex_saving_reward_rate.is_zero() {
			if let Some((currency_id_a, currency_id_b)) = lp_currency_id.split_dex_share_currency_id() {
				...
				if !dex_saving_reward_amount.is_zero() {
					...
					match res {
						Ok(_) => {
							<orml_rewards::Pallet<T>>::accumulate_reward(&pool_id, dex_saving_reward_amount);
						}
						Err(e) => {
							log::warn!(
								target: "incentives",
...
impl<T: Config> Happened<(T::AccountId, CurrencyId, Amount, Balance)> for OnUpdateLoan<T> {
	fn happened(info: &(T::AccountId, CurrencyId, Amount, Balance)) {
		...
		if !adjustment_abs.is_zero() {
			...
			<orml_rewards::Pallet<T>>::set_share(
				who,
				&PoolId::LoansIncentive {
					collateral_currency_id: *currency_id,
					reward_currency_id: T::NativeCurrencyId::get(),
				},
				new_share_amount,
			);
		}
