	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn add_liquidity(
			...
			if total_shares.is_zero() {
				...
				let other_base_price = Price::saturating_from_rational(base_currency_pool, other_currency_pool);
				let input_other_base_price = Price::saturating_from_rational(max_base_currency_amount, max_other_currency_amount);
				...
				if input_other_base_price <= other_base_price {
					...
					let base_other_price = Price::saturating_from_rational(other_currency_pool, base_currency_pool);
					let other_currency_amount = base_other_price.saturating_mul_int(max_base_currency_amount);
					let share = Ratio::saturating_from_rational(other_currency_amount, other_currency_pool).checked_mul_int(total_shares).unwrap_or_default();
					...
					let share = Ratio::saturating_from_rational(base_currency_amount, base_currency_pool).checked_mul_int(total_shares).unwrap_or_default();
		...
		pub fn withdraw_liquidity(origin, currency_id: CurrencyId, #[compact] share_amount: T::Share) {
			...
			let proportion = Ratio::saturating_from_rational(share_amount, Self::total_shares(currency_id));
...
impl<T: Trait> Module<T> {
	...
	pub fn calculate_swap_target_amount(supply_pool: Balance, target_pool: Balance, supply_amount: Balance) -> Balance {
		...
		let new_target_pool = supply_pool
			...
			.map(|n| Ratio::saturating_from_rational(supply_pool, n))
	...
	pub fn deposit_calculate_interest(currency_id: CurrencyId, who: &T::AccountId, share_amount: T::Share) {
		...
		let proportion = Ratio::saturating_from_rational(share_amount, total_shares);
	...
	fn withdraw_calculate_interest(
		...
		let proportion = Ratio::saturating_from_rational(share_amount, Self::total_shares(currency_id));
		let withdrawn_interest_to_remove =
			Ratio::saturating_from_rational(share_amount, Self::shares(currency_id, who))
	...
	fn claim_interest(currency_id: CurrencyId, who: &T::AccountId) -> DispatchResult {
		let proportion =
			Ratio::saturating_from_rational(Self::shares(currency_id, who), Self::total_shares(currency_id));
...
impl<T: Trait> DEXManager<T::AccountId, CurrencyId, Balance> for Module<T> {
	...
	fn get_exchange_slippage(
		...
		if supply_currency_id == target_currency_id {
			...
			let supply_to_base_slippage: Ratio = Ratio::saturating_from_rational(
				supply_amount,
				supply_amount.saturating_add(supply_other_currency_pool),
			);
			...
			let base_to_target_slippage: Ratio = Ratio::saturating_from_rational(
