...
fn end_provisioning_trading_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Tokens::total_issuance((*AUSD_BTC_PAIR).dex_share_currency_id()), 0);
		assert_eq!(
			Tokens::free_balance((*AUSD_BTC_PAIR).dex_share_currency_id(), &DexModule::account_id()),
			0
		);
		...
		assert_eq!(
			Tokens::total_issuance((*AUSD_BTC_PAIR).dex_share_currency_id()),
			4_000_000_000_000u128
		);
		assert_eq!(
			Tokens::free_balance((*AUSD_BTC_PAIR).dex_share_currency_id(), &DexModule::account_id()),
...
fn claim_dex_share_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let lp_currency_id = (*AUSD_DOT_PAIR).dex_share_currency_id();
...
fn add_liquidity_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Tokens::free_balance(DOT, &DexModule::account_id()), 0);
			assert_eq!(
				Tokens::free_balance((*AUSD_DOT_PAIR).dex_share_currency_id(), &ALICE),
				0
			);
			assert_eq!(
				Tokens::reserved_balance((*AUSD_DOT_PAIR).dex_share_currency_id(), &ALICE),
				0
			);
			...
			assert_eq!(Tokens::free_balance((*AUSD_DOT_PAIR).dex_share_currency_id(), &BOB), 0);
			assert_eq!(
				Tokens::reserved_balance((*AUSD_DOT_PAIR).dex_share_currency_id(), &BOB),
				0
			);
		...
		.execute_with(|| {
			...
			assert_noop!(
				DexModule::remove_liquidity(
					Origin::signed(ALICE),
					(*AUSD_DOT_PAIR).dex_share_currency_id(),
					DOT,
					100_000_000,
					0,
					0,
