...
pub mod module {
	...
	pub trait Config: frame_system::Config {
		...
		/// The RelaychainInterface to manage the staking of sub-account on relaychain.
		type RelaychainInterface: HomaSubAccountXcm<Self::AccountId, Balance>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn process_scheduled_unbond(new_era: EraIndex) -> DispatchResult {
			...
			for (sub_account_index, ledger) in StakingLedgers::<T>::iter() {
				...
				if !expired_unlocking.is_zero() {
					T::RelaychainInterface::withdraw_unbonded_from_sub_account(sub_account_index, expired_unlocking)?;
		...
		pub fn process_to_bond_pool() -> DispatchResult {
			...
			if to_bond_pool >= T::MintThreshold::get() {
				let xcm_transfer_fee = T::RelaychainInterface::get_xcm_transfer_fee();
				...
				for (sub_account_index, amount) in distribution {
					if !amount.is_zero() {
						T::RelaychainInterface::transfer_staking_to_sub_account(
							&Self::account_id(),
							sub_account_index,
							amount,
						)?;
						...
						T::RelaychainInterface::bond_extra_on_sub_account(sub_account_index, bond_amount)?;
		...
		pub fn process_redeem_requests(new_era: EraIndex) -> DispatchResult {
			...
			for (sub_account_index, unbond_amount) in distribution {
				if !unbond_amount.is_zero() {
					T::RelaychainInterface::unbond_on_sub_account(sub_account_index, unbond_amount)?;
