...
pub mod module {
	...
	pub struct CollateralAuctionItem<AccountId, BlockNumber> {
		...
		/// bidder
		#[codec(compact)]
		pub(crate) target: Balance,
		/// Auction start time
		pub(crate) start_time: BlockNumber,
	}
	...
	impl<AccountId, BlockNumber> CollateralAuctionItem<AccountId, BlockNumber> {
		...
		pub(crate) fn always_forward(&self) -> bool {
			self.target.is_zero()
		}

		/// Return whether the collateral auction is in reverse stage at
		/// specific bid price
		pub(crate) fn in_reverse_stage(&self, bid_price: Balance) -> bool {
			!self.always_forward() && bid_price >= self.target
		}

		/// Return the actual number of stablecoins to be paid
		pub(crate) fn payment_amount(&self, bid_price: Balance) -> Balance {
		...
		/// price
		pub(crate) fn collateral_amount(&self, last_bid_price: Balance, new_bid_price: Balance) -> Balance {
	...
	impl<BlockNumber> DebitAuctionItem<BlockNumber> {
		...
		pub(crate) fn amount_for_sale(&self, last_bid_price: Balance, new_bid_price: Balance) -> Balance {
	...
	impl<T: Config> Pallet<T> {
		pub(crate) fn get_last_bid(auction_id: AuctionId) -> Option<(T::AccountId, Balance)> {
		...
		pub(crate) fn cancel_surplus_auction(
			id: AuctionId,
		...
		pub(crate) fn cancel_debit_auction(
			id: AuctionId,
		...
		pub(crate) fn cancel_collateral_auction(
			id: AuctionId,
		...
		pub(crate) fn get_auction_time_to_close(now: T::BlockNumber, start_block: T::BlockNumber) -> T::BlockNumber {
			if now >= start_block + T::AuctionDurationSoftCap::get() {
