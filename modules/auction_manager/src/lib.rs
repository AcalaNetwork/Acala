...
impl<T: Trait> Module<T> {
	...
	pub fn collateral_auction_end_handler(auction_id: AuctionIdOf<T>, winner: Option<(T::AccountId, BalanceOf<T>)>) {
		if let (Some(collateral_auction), Some((bidder, bid_price))) = (Self::collateral_auctions(auction_id), winner) {
			...
			.is_ok()
			{
				...
				<Module<T>>::deposit_event(RawEvent::CollateralAuctionDealed(
					...
					collateral_auction.amount,
					bidder,
					rstd::cmp::min(collateral_auction.target, bid_price),
				));
			}
		}
	}

	pub fn debit_auction_end_handler(auction_id: AuctionIdOf<T>, winner: Option<(T::AccountId, BalanceOf<T>)>) {
	...
	pub fn surplus_auction_end_handler(auction_id: AuctionIdOf<T>, winner: Option<(T::AccountId, BalanceOf<T>)>) {
