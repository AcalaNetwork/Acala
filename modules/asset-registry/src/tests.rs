...
#[test]
fn set_foreign_asset_extra_metadata_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			AssetRegistry::set_foreign_asset_extra_metadata(Origin::signed(CouncilAccount::get()), 0, Box::new(None)),
			Error::<Runtime>::AssetIdNotExists
		);
		...
		assert_eq!(
			AssetIdMaps::<Runtime>::get_currency_id(location.clone()),
			Some(CurrencyId::ForeignAsset(0))
		);
		assert_eq!(AssetIdMaps::<Runtime>::get_multi_location(0), Some(location.clone()));

		assert_noop!(
			AssetRegistry::set_foreign_asset_extra_metadata(Origin::signed(alice()), 0, Box::new(None)),
			BadOrigin
		);
		...
		assert_ok!(AssetRegistry::set_foreign_asset_extra_metadata(
			...
			Box::new(Some(ForeignAssetExtraMetadata {
				override_currency_id: Some(CurrencyId::Token(TokenSymbol::ACA)),
				override_multi_location: None,
			}))
		));
		System::assert_last_event(Event::AssetRegistry(crate::Event::ForeignAssetExtraMetadataSet {
			asset_id: 0,
			metadata: Some(ForeignAssetExtraMetadata {
				override_currency_id: Some(CurrencyId::Token(TokenSymbol::ACA)),
				override_multi_location: None,
			}),
		}));

		// Currency ID can be overriden
		assert_eq!(
			AssetIdMaps::<Runtime>::get_currency_id(location.clone()),
			Some(CurrencyId::Token(TokenSymbol::ACA))
		);
		...
		assert_ok!(AssetRegistry::set_foreign_asset_extra_metadata(
			...
			0,
			Box::new(Some(ForeignAssetExtraMetadata {
				override_currency_id: None,
				override_multi_location: Some(MultiLocation::here()),
			}))
		));

		// Location can be overriden
		assert_eq!(
			AssetIdMaps::<Runtime>::get_currency_id(location.clone()),
			Some(CurrencyId::ForeignAsset(0))
		);
		assert_eq!(
			AssetIdMaps::<Runtime>::get_multi_location(0),
			Some(MultiLocation::here())
		);

		// Extra metadata can be removed.
		assert_ok!(AssetRegistry::set_foreign_asset_extra_metadata(
			Origin::signed(CouncilAccount::get()),
			0,
			Box::new(None)
