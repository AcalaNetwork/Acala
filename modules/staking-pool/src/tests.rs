...
fn mint_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		System::assert_last_event(Event::StakingPoolModule(crate::Event::MintLiquid {
			who: ALICE,
			staking_amount_deposited: 500,
			liquid_amount_issued: 5000,
		}));
...
fn redeem_by_unbond_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		System::assert_last_event(Event::StakingPoolModule(crate::Event::RedeemByUnbond {
			who: ALICE,
			liquid_amount_burned: 1000,
			staking_amount_redeemed: 100,
		}));
		...
		System::assert_last_event(Event::StakingPoolModule(crate::Event::RedeemByUnbond {
			who: BOB,
			liquid_amount_burned: 4000,
			staking_amount_redeemed: 400,
		}));
...
fn redeem_by_free_unbonded_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		System::assert_last_event(Event::StakingPoolModule(crate::Event::RedeemByFreeUnbonded {
			who: ALICE,
			fee_in_staking: 1000,
			liquid_amount_burned: 80,
			staking_amount_redeemed: 20,
		}));
		...
		System::assert_last_event(Event::StakingPoolModule(crate::Event::RedeemByFreeUnbonded {
			who: BOB,
			fee_in_staking: 3662,
			liquid_amount_burned: 300,
			staking_amount_redeemed: 74,
		}));
...
fn redeem_by_claim_unbonding_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		System::assert_last_event(Event::StakingPoolModule(crate::Event::RedeemByClaimUnbonding {
			...
			target_era: 4,
			fee_in_staking: 3910,
			liquid_amount_burned: 316,
			staking_amount_redeemed: 79,
		}));
		assert_eq!(
			StakingPoolModule::staking_pool_ledger(),
			Ledger {
