...
impl<T: Trait> Module<T> {
	...
	//         (if the transfer is possible). Consider a new name like `try_record_free_transfer`.
	...
	fn open_account(k: &T::AccountId) {
		...
		if <T as Trait>::Currency::reserve(native_currency_id, k, T::NewAccountDeposit::get()).is_ok() {
			...
			if *k != treasury_account {
				...
				// REVIEW: IIUC the account could have lots of reserved balance, just not any free
				//         balance and thus should maybe not be considered dust just based on the
				//         reserving failing?
