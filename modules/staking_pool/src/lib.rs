...
pub mod module {
	...
	use sp_std::prelude::*;
	use support::{
		ExchangeRate, HomaProtocol, NomineesProvider, OnNewEra, PolkadotBridge, PolkadotBridgeCall,
		PolkadotBridgeState, PolkadotBridgeType, PolkadotStakingLedger, PolkadotUnlockChunk, Rate, Ratio,
	};
	...
	impl Ledger {
		...
		/// holders.
		pub(crate) fn total_belong_to_liquid_holders(&self) -> Balance {
		...
		pub(crate) fn unbonding_to_free_ratio(&self) -> Ratio {
	...
	pub(crate) type ChangeRate = Change<Rate>;
	pub(crate) type ChangeRatio = Change<Ratio>;
			...
			StakingPoolLedger::<T>::mutate(|ledger| {
				...
				if !total_unbond.is_zero() {
					...
					if Self::unbond(total_unbond).is_ok() {
						let expired_era_index = current_era.saturating_add(
		...
		fn redeem_by_claim_unbonding(
			...
			StakingPoolLedger::<T>::try_mutate(|ledger| -> DispatchResult {
				...
				if !demand_staking_amount.is_zero() && !available_unclaimed_unbonding.is_zero() {
					...
					let current_unclaimed_ratio = Ratio::checked_from_rational(unclaimed, initial_unclaimed).expect(
						"if available_unclaimed_unbonding is not zero, initial_unclaimed must not be zero; qed",
					);
