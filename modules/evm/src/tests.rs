...
fn should_create_and_call_contract() {
	...
	new_test_ext().execute_with(|| {
		// deploy contract
		let caller = alice();
		let result = <Test as Config>::Runner::create(
			caller.clone(),
			contract,
			0,
			1000000,
...
fn create_reverts_with_message() {
	...
	new_test_ext().execute_with(|| {
		let result = <Test as Config>::Runner::create(alice(), contract, 0, 12_000_000).unwrap();
...
fn should_deploy_payable_contract() {
	...
	new_test_ext().execute_with(|| {
		...
		let result = <Test as Config>::Runner::create(alice(), contract, amount, 100000).unwrap();
		let contract_address = result.address;
...
fn should_transfer_from_contract() {
	...
	new_test_ext().execute_with(|| {
		...
		let result = <Test as Config>::Runner::create(alice(), contract, 0, 10000000).expect("create shouldn't fail");
		assert_eq!(result.exit_reason, ExitReason::Succeed(ExitSucceed::Returned));
		let contract_address = result.address;
		...
		let result = <Test as Config>::Runner::call(alice(), contract_address, via_transfer, amount, 1000000)
		...
		let result = <Test as Config>::Runner::call(alice(), contract_address, via_send, amount, 1000000)
			.expect("call shouldn't fail");
		...
		let result = <Test as Config>::Runner::call(alice(), contract_address, via_call, amount, 1000000)
			.expect("call shouldn't fail");
...
fn contract_should_deploy_contracts() {
	...
	new_test_ext().execute_with(|| {
		let result = <Test as Config>::Runner::create(alice(), contract.clone(), 0, 1000000000).unwrap();
		assert_eq!(result.exit_reason, ExitReason::Succeed(ExitSucceed::Returned));
		assert_eq!(
			balance(alice()),
		...
		let result =
			<Test as Config>::Runner::call(alice(), result.address, create_contract, amount, 1000000000).unwrap();
		assert_eq!(result.exit_reason, ExitReason::Succeed(ExitSucceed::Stopped));
		assert_eq!(
			balance(alice()),
			INITIAL_BALANCE - amount - <Test as Config>::ContractExistentialDeposit::get()
...
fn contract_deploy_contracts_failed() {
	...
	new_test_ext().execute_with(|| {
		...
		// payable.
		let create_contract = from_hex("0x412a5a6d").unwrap();
		let result = <Test as Config>::Runner::call(alice(), result.address, create_contract, 0, 1000000000).unwrap();
		assert_eq!(result.exit_reason, ExitReason::Revert(ExitRevert::Reverted));
	});
}
