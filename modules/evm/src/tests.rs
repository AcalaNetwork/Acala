...
fn fail_call_return_ok() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(EVM::call(Origin::signed(signer), bob(), Vec::new(), 0, 1000000,));
	});
}
...
fn create_reverts_with_message() {
	...
	new_test_ext().execute_with(|| {
		let result =
			<Test as Config>::Runner::create(alice(), contract, 0, 12_000_000, <Test as Config>::config()).unwrap();
...
fn should_deploy_payable_contract() {
	...
	new_test_ext().execute_with(|| {
		...
		let result =
			<Test as Config>::Runner::create(alice(), contract, amount, 100000, <Test as Config>::config()).unwrap();
		let contract_address = result.address;
...
fn should_transfer_from_contract() {
	...
	new_test_ext().execute_with(|| {
		...
		let result = <Test as Config>::Runner::create(alice(), contract, 0, 10000000, <Test as Config>::config())
...
fn contract_should_deploy_contracts() {
	...
	new_test_ext().execute_with(|| {
		let result =
			<Test as Config>::Runner::create(alice(), contract.clone(), 0, 1000000000, <Test as Config>::config())
				.unwrap();
		assert_eq!(result.exit_reason, ExitReason::Succeed(ExitSucceed::Returned));
		assert_eq!(
			balance(alice()),
...
fn should_add_and_remove_storage_quota() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			...
			INITIAL_BALANCE
				- amount - <Test as Config>::ContractExistentialDeposit::get()
				- ((add_storage_quota as u128) * <Test as Config>::StorageDepositPerByte::get())
		);
