...
// The URL for the mandala telemetry server.
const MANDALA_TELEMETRY_URL: &str = "wss://telemetry.polkadot.io/submit/";
...
/// The `ChainSpec parametrised for mandala runtime`.
pub type MandalaChainSpec = sc_service::GenericChainSpec<mandala_runtime::GenesisConfig, Extensions>;
...
/// Mandala development testnet config (single validator Alice)
pub fn development_testnet_config() -> Result<MandalaChainSpec, String> {
	...
	Ok(MandalaChainSpec::from_genesis(
		"Mandala Development",
		"mandala_development",
...
/// Mandala Local testnet config (multivalidator Alice + Bob)
pub fn local_testnet_config() -> Result<MandalaChainSpec, String> {
	let mut properties = Map::new();
	properties.insert("tokenSymbol".into(), "ACA".into());
	properties.insert("tokenDecimals".into(), 18.into());
...
pub fn latest_testnet_config() -> Result<MandalaChainSpec, String> {
	let mut properties = Map::new();
	properties.insert("tokenSymbol".into(), "ACA".into());
	properties.insert("tokenDecimals".into(), 18.into());
	...
	Ok(MandalaChainSpec::from_genesis(
		...
		TelemetryEndpoints::new(vec![(MANDALA_TELEMETRY_URL.into(), 0)]).ok(),
		Some("mandala5"),
		Some(properties),
		Default::default(),
	))
...
pub fn testnet_config() -> Result<MandalaChainSpec, String> {
	MandalaChainSpec::from_json_bytes(&include_bytes!("../../resources/mandala-dist.json")[..])
}
