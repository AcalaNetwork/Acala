...
// The URL for the acala telemetry server.
const ACALA_TELEMETRY_URL: &str = "wss://telemetry.polkadot.io/submit/";
...
/// The `ChainSpec parametrised for acala runtime`.
pub type AcalaChainSpec = sc_service::GenericChainSpec<acala_runtime::GenesisConfig, Extensions>;

fn acala_session_keys(grandpa: GrandpaId, babe: BabeId) -> acala_runtime::SessionKeys {
	acala_runtime::SessionKeys { grandpa, babe }
}
...
/// Acala development testnet config (single validator Alice)
pub fn development_testnet_config() -> Result<AcalaChainSpec, String> {
	let mut properties = Map::new();
	properties.insert("tokenSymbol".into(), "ACA".into());
	properties.insert("tokenDecimals".into(), 18.into());

	let wasm_binary = acala_runtime::WASM_BINARY.ok_or("Dev runtime wasm binary not available")?;

	Ok(AcalaChainSpec::from_genesis(
		"Acala Development",
		"acala_development",
...
/// Acala Local testnet config (multivalidator Alice + Bob)
pub fn local_testnet_config() -> Result<AcalaChainSpec, String> {
	let mut properties = Map::new();
	properties.insert("tokenSymbol".into(), "ACA".into());
	properties.insert("tokenDecimals".into(), 18.into());
...
/// Latest Acala testnet config
pub fn latest_testnet_config() -> Result<AcalaChainSpec, String> {
	let mut properties = Map::new();
	properties.insert("tokenSymbol".into(), "ACA".into());
	properties.insert("tokenDecimals".into(), 18.into());
	...
	Ok(AcalaChainSpec::from_genesis(
		...
		TelemetryEndpoints::new(vec![(ACALA_TELEMETRY_URL.into(), 0)]).ok(),
		Some("acala_tc5"),
		Some(properties),
		Default::default(),
	))
