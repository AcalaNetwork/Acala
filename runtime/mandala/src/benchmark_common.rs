...
use crate::{
	...
	TreasuryPalletId, EVM, MILLISECS_PER_BLOCK
};
use alloc::boxed::Box;
use alloc::vec::Vec;
...
use runtime_common::{LCDOT, TokenInfo};
	...
	T: module_cdp_engine::Config,
{
	fn setup_on_initialize(c: u32) -> Option<BlockNumberFor<T>> {
		...
		let currency_ids = super::get_benchmarking_collateral_currency_ids();
	...
	T: module_currencies::Config,
{
	...
	fn setup_get_treasury_account() -> Option<AccountId> {
		Some(TreasuryPalletId::get().into_account_truncating())
	}
}
	...
	T: frame_system::Config<AccountId = AccountId> + pallet_balances::Config<Balance = Balance> + orml_vesting::Config,
{
	fn get_vesting_account_and_amount() -> Option<(AccountId, <T as pallet_balances::Config>::Balance)> {
		Some((super::get_vesting_account(), dollar(NATIVE)))
	}
}
