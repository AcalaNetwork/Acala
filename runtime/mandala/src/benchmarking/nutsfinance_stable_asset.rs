...
use primitives::currency::{ACA, AUSD, RENBTC};
use sp_std::prelude::*;
...
runtime_benchmarks! {
	...
	create_pool {
		...
		let pool_asset = RENBTC;
		let assets = vec![ACA, AUSD];
	...
	mint {
		...
		set_balance_fungibles(ACA, &tester, 100000000u128);
		set_balance_fungibles(AUSD, &tester, 100000000u128);
