...
runtime_benchmarks! {
	...
	deposit_dex_share {
		...
		set_balance(ACA_AUSD_LP, &caller, 10_000 * dollar(AUSD));
	}: _(RawOrigin::Signed(caller), ACA_AUSD_LP, 10_000 * dollar(AUSD))
	...
	}: _(RawOrigin::Signed(caller), ACA_AUSD_LP, 8000 * dollar(AUSD))
