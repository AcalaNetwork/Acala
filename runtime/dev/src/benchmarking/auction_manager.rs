use crate::{
	AcalaOracle, AccountId, AuctionId, AuctionManager, Balance, CurrencyId, EmergencyShutdown, GetNativeCurrencyId,
	GetStableCurrencyId, Price, Runtime, TokenSymbol, DOLLARS,
};
...
runtime_benchmarks! {
	...
	cancel_collateral_auction {
		...
		// set balance
		<Runtime as module_auction_manager::Trait>::Currency::deposit(stable_currency_id, &bidder, dollar(80))?;
		<Runtime as module_auction_manager::Trait>::Currency::deposit(CurrencyId::Token(TokenSymbol::DOT), &funder, dollar(1))?;
		<Runtime as module_auction_manager::Trait>::CDPTreasury::deposit_collateral(&funder, CurrencyId::Token(TokenSymbol::DOT), dollar(1))?;
