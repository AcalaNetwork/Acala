use crate::{AcalaOracle, AccountId, Balance, CollateralCurrencyIds, EmergencyShutdown, Price, Runtime, DOLLARS};
...
runtime_benchmarks! {
	...
	refund_collaterals {
		...
		for i in 0 .. c {
			...
			<Runtime as module_emergency_shutdown::Trait>::CDPTreasury::deposit_collateral(&funder, currency_id, dollar(100))?;
		}
		AcalaOracle::feed_values(RawOrigin::Root.into(), values)?;

		<Runtime as module_emergency_shutdown::Trait>::CDPTreasury::issue_debit(&caller, dollar(1000), true)?;
		<Runtime as module_emergency_shutdown::Trait>::CDPTreasury::issue_debit(&funder, dollar(9000), true)?;
