...
use crate::{
	dollar, AcalaOracle, AccountId, CdpTreasury, CollateralCurrencyIds, EmergencyShutdown, Price, Runtime, KUSD,
};
...
runtime_benchmarks! {
	...
	refund_collaterals {
		...
		CdpTreasury::issue_debit(&caller, 1_000 * dollar(KUSD), true)?;
		CdpTreasury::issue_debit(&funder, 1_000 * dollar(KUSD), true)?;

		EmergencyShutdown::emergency_shutdown(RawOrigin::Root.into())?;
		EmergencyShutdown::open_collateral_refund(RawOrigin::Root.into())?;
	}: _(RawOrigin::Signed(caller),  1_000 * dollar(KUSD))
}
