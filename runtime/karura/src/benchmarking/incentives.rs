...
const SEED: u32 = 0;
const BTC_AUSD_LP: CurrencyId = CurrencyId::DEXShare(TokenSymbol::XBTC, TokenSymbol::KUSD);
...
runtime_benchmarks! {
	...
	update_loans_incentive_rewards {
		...
		for i in 0 .. c {
			...
			values.push((currency_id, 100 * dollar(KAR)));
		}
	...
	update_dex_incentive_rewards {
		...
		for i in 0 .. c {
			...
			values.push((lp_share_currency_id, 100 * dollar(KAR)));
		}
	}: _(RawOrigin::Root, values)

	update_homa_incentive_reward {
	}: _(RawOrigin::Root, 100 * dollar(KAR))
