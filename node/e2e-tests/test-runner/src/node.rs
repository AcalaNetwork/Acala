...
pub struct Node<T: ChainInfo<Block = node_primitives::Block>> {
...
impl<T> Node<T>
where
	T: ChainInfo<Block = node_primitives::Block>,
{
	/// Creates a new node.
	pub fn new(
		rpc_handler: Arc<MetaIoHandler<sc_rpc::Metadata, sc_rpc_server::RpcMiddleware>>,
...
impl<T: ChainInfo<Block = node_primitives::Block>> Drop for Node<T> {
	fn drop(&mut self) {
