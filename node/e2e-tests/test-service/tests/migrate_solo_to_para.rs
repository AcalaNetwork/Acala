...
//! the solo chain is being stopped at a given point and sends its last header to the running
//! parachain. The parachain will return this header as part of the validation phase on the relay
//! chain to enact this header as its current latest state. As the old running parachain doesn't
//! know this header, it will stop to produce new blocks. However, the old solo chain can now
//! produce blocks using the parachain slot. (Be aware, that this is just a highlevel description
//! and some parts are omitted.)
...
async fn test_migrate_solo_to_para() {
	...
	let parachain = test_service::TestNodeBuilder::new(para_id, tokio_handle.clone(), Charlie)
		.enable_collator()
		.connect_to_relay_chain_nodes(vec![&alice, &bob])
		.build()
		.await;

	// run the solo chain (in our case this is also already a parachain, but as it has a different
	// genesis it will not produce any blocks.)
	let solo = test_service::TestNodeBuilder::new(para_id, tokio_handle, Dave)
		.enable_collator()
		.connect_to_relay_chain_nodes(vec![&alice, &bob])
	...
	// Wait for some more blocks and check that the old parachain doesn't produced/imported any new
	// blocks.
